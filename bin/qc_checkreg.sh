...
if [ $failMode -eq 1 ]; then
	...
	for reg in data/preproc/task-${task}/acq-${acq}/sub-${sub}/ses-${ses}/run-${run}/QC/checkreg.done; do
	...
	#get rating from file
		...
		if [ ${regRating} -eq 0 ]; then
			...
			qcDir="${preprocDir}/QC"
			bolds+=(${preprocDir}/*_utboldref.nii.gz)
			t1ws+=("${preprocDir}/chT1w.nii")
			lockFile="${qcDir}/checkreg.lock"
			locks+=(${lockFile})
			#lock the subject
	    echo "${USER}" > "${lockFile}"
	    echo "$(date)" >> "${lockFile}"
	...
	for bold in data/preproc/task-${task}/acq-${acq}/sub-${sub}/ses-${ses}/run-${run}/*_utboldref.nii.gz; do
	  ...
	  t1w="$(realpath ${preprocDir}/..)/cuhT1w.nii"
	  qcDir="${preprocDir}/QC"
	  lockFile="${qcDir}/checkreg.lock"
	  doneFile="${qcDir}/checkreg.done"
...
for i in $(seq 0 $nCheck); do
  ...
  c="task-$(echo ${bolds[i]#*"task-"} | cut -d "/" -f 1)"
...
while IFS=',' read func rating flag user check; do
	if [[ "${func}" == *"utboldref.nii"* ]]; then
		...
		qcFile="data/preproc/task-${task}/acq-${acq}/sub-${sub}/ses-${ses}/run-${run}/QC/${check}.done"
